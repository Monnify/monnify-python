...
class TestTransactionAPIs:
    ...
    def instantiate_class(self):
        self.__instance = Transaction(os.environ.get('API_KEY'),os.environ.get('SECRET_KEY'))
    ...
    def test_transaction_status_v2(self,token):
         code, result = self.__instance. get_transaction_status_v2(token,transaction_reference=self.__trx_ref)
         assert code == 200 and result['responseBody']['paymentStatus'] == 'PAID'
         
    def test_transaction_status_v1(self,token):
            code, result = self.__instance. get_transaction_status_v1(token,transaction_reference=self.__trx_ref)
            assert code == 200 and result['responseBody']['paymentStatus'] == 'PAID'

            code, result = self.__instance. get_transaction_status_v1(token,payment_reference=self.__payment_ref)
            assert code == 200 and result['responseBody']['paymentStatus'] == 'PAID'

            with pytest.raises((UnprocessableRequestException,Exception)):
                self.__instance. get_transaction_status_v1(token,payment_reference=self.__trx_ref)
...
class TestReservedAccountAPIs:
    ...
    @pytest.fixture(autouse=True)
    def init_class(self):
        self.__instance = ReservedAccount(os.environ.get('API_KEY'),os.environ.get('SECRET_KEY'))
    ...
    def test_add_linked_account(self, token, get_account_reference):
        ...
        data = {"getAllAvailableBanks":False, "preferredBanks":["232"], "accountReference":get_account_reference}
    ...
    def test_account_details(self, token, get_account_reference):
        code, result = self.__instance.get_reserved_account_details(token, get_account_reference)
        assert code == 200

    def test_account_transactions(self, token, get_account_reference):
        code, result = self.__instance.get_reserved_account_transactions(token, get_account_reference)
        assert code == 200

    def test_account_deallocation(self, token, get_account_reference):
        code, result = self.__instance.deallocate_reserved_account(token, get_account_reference)
        assert code == 200
...
class TestInvoiceAPIs:
    ...
    @pytest.fixture(autouse=True)
    def instantiate_class(self):
        self.__instance = Invoice(os.environ.get('API_KEY'),os.environ.get('SECRET_KEY'))
