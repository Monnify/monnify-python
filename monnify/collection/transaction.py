...
class Transaction(Base):
    ...
    def initialize_transaction(self, auth_token, data) -> tuple:
        ...
        # Todo, add validation with marshmallow
    ...
    def get_transaction_status_v1(self, auth_token, payment_reference=None, transaction_reference=None) -> tuple:

        if(payment_reference is None and transaction_reference is None):
            raise Exception("At least one of payment or transaction reference is required!!")
        
        url_path = ("/api/v2/merchant/transactions/query?transactionReference="+url_encoder.quote_plus(transaction_reference) 
                    if(payment_reference is None) 
                    else "/api/v2/merchant/transactions/query?paymentReference="+url_encoder.quote_plus(payment_reference))
        return self.do_get(url_path, auth_token)
